...
// Intelligent CSV row processing with proper field detection
function processCsvRow(row: any, broker: string, userId: string, fieldMap?: Record<string, string>): any | null {
  try {
    ...
    if (rowText.includes('conta:') || rowText.includes('header') || rowText.includes('campo') || 
        rowText.includes('_1') || rowText.includes('_2') || rowText.includes('field')) {
      console.log('Skipping header/account row:', Object.values(row).slice(0, 3));
      return null;
    }

    console.log('Processando dados da linha:', Object.values(row).slice(0, 5));

    // Identify CSV structure by analyzing the row data
    const rowValues = Object.values(row);
    const rowKeys = Object.keys(row);
    
    // Find potential fields by their content patterns
    const fields = {
      date: null as string | null,
      symbol: null as string | null,
      type: null as string | null,
      openPrice: null as string | null,
    ...
    // Analyze each field to identify its purpose
    rowValues.forEach((value, index) => {
      const key = rowKeys[index];
      const strValue = value?.toString() || '';
      
      // Date detection - various formats
      if (/\d{4}[-\/]\d{2}[-\/]\d{2}|\d{2}[-\/]\d{2}[-\/]\d{4}/.test(strValue)) {
        fields.date = key;
      }
      
      // Symbol detection - trading instruments
      else if (/^[A-Z]{3,6}$|^[A-Z]+\d+$|\//.test(strValue) && strValue.length <= 10) {
        fields.symbol = key;
      }
      
      // Trade type detection
      else if (/^(buy|sell|long|short|compra|venda)$/i.test(strValue)) {
        fields.type = key;
      }
      
      // Price detection - reasonable numeric values
      else if (/^\d+\.?\d*$/.test(strValue)) {
        const num = parseFloat(strValue);
        if (num > 0 && num <= 100000) {
          if (!fields.openPrice) fields.openPrice = key;
          else if (!fields.closePrice) fields.closePrice = key;
        }
      }
      
      // Profit/Loss detection - can be negative
      else if (/^-?\d+\.?\d*$/.test(strValue)) {
        const num = parseFloat(strValue);
        if (Math.abs(num) <= 50000) {
          if (!fields.profit) fields.profit = key;
          else if (!fields.stopLoss) fields.stopLoss = key;
        }
      }
      
      // Quantity detection - usually small positive numbers
      else if (/^\d+\.?\d*$/.test(strValue)) {
        const num = parseFloat(strValue);
        if (num > 0 && num <= 1000) {
          if (!fields.quantity) fields.quantity = key;
        }
      }
    });

    console.log('Campos identificados:', fields);

    // Extract and validate data
    const dateStr = fields.date ? row[fields.date] : null;
    const symbolStr = fields.symbol ? row[fields.symbol] : 'UNKNOWN';
    const typeStr = fields.type ? row[fields.type] : 'buy';
    const openPriceStr = fields.openPrice ? row[fields.openPrice] : '0';
    const closePriceStr = fields.closePrice ? row[fields.closePrice] : openPriceStr;
    const quantityStr = fields.quantity ? row[fields.quantity] : '1';
    const profitStr = fields.profit ? row[fields.profit] : '0';
    const stopLossStr = fields.stopLoss ? row[fields.stopLoss] : '0';

    // Validate that we have minimum required data
    if (!symbolStr || symbolStr === 'UNKNOWN' || !profitStr) {
    ...
    const openPrice = Math.max(0, parseFloat(openPriceStr) || 0);
    const closePrice = Math.max(0, parseFloat(closePriceStr) || openPrice);
    const quantity = Math.max(0.001, parseFloat(quantityStr) || 1);
    const profit = parseFloat(profitStr) || 0;
    const stopLoss = Math.abs(parseFloat(stopLossStr) || 0);

    // Calculate R/R ratio if we have profit and stop loss
    let rrRatio = 0;
    if (stopLoss > 0 && profit !== 0) {
      rrRatio = Math.abs(profit) / stopLoss;
    }

    // Determine market type
    let detectedMarket = 'forex';
    const symbolUpper = symbolStr.toUpperCase();
    if (symbolUpper.includes('WIN') || symbolUpper.includes('WDO') || symbolUpper.match(/[A-Z]{4}\d{2}/)) {
      detectedMarket = 'b3';
    } else if (symbolUpper.includes('BTC') || symbolUpper.includes('ETH') || symbolUpper.includes('USDT')) {
    ...
    const trade: any = {
      ...
      comentario: rrRatio > 0 ? `R/R: ${rrRatio.toFixed(2)}` : 'CSV Import'
    };
...
// Remove the old duplicate code below

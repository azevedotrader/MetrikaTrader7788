...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  app.post("/api/trades/upload-csv", requireAuth, upload.single('csvFile'), async (req, res) => {
    try {
      ...
      // SEMPRE usar sistema tradicional primeiro (ULTRA-RÁPIDO) 
      console.log(`⚡ Usando Sistema Tradicional (ULTRA-RÁPIDO) por padrão...`);
      ...
      try {
        ...
        if (result.trades && result.trades.length > 0) {
          console.log(`✅ Sistema tradicional processou com sucesso: ${result.trades.length} trades`);
          // Continuar com o processamento normal
        } else {
          throw new Error('Sistema tradicional não encontrou trades válidos');
        }
        
      } catch (traditionalError) {
        console.log(`⚠️ Sistema tradicional falhou: ${traditionalError.message}`);
        
        // Só usar ChatGPT como último recurso
        if (!useTraditional) {
          console.log(`🤖 Fallback para ChatGPT (pode ser lento)...`);
        ...
        // Sistema simplificado - apenas usar sistema tradicional
        console.log(`⚡ Usando apenas sistema tradicional por performance...`);
        const { processSmartCSV } = await import('./smart-csv-processor');
        result = await processSmartCSV(file.path, userId, broker);
        ...
        if (!result.summary) {
          result.summary = {
            totalRows: 0,
            tradesFound: result.trades?.length || 0,
            statisticsSkipped: 0,
            dateRange: null,
            detectedBroker: broker,
            detectedMarket: broker
          };
        }
        result.summary.processingMethod = 'Sistema Tradicional (Ultra-Rápido)';

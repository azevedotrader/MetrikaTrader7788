...
/**
 * Demonstração completa e robusta da função parseCSV
 * Cobre cenários de diferentes formatos, delimitadores, encodings e erros.
 */

console.log("🎯 === DEMONSTRAÇÃO COMPLETA parseCSV ===\n");

/**
 * 1. CSV Brasileiro típico da B3
 */
...
console.log(
  `✅ Dados limpos: ${resultMalformado.meta.totalRows} registros válidos`,
);
...
console.log(
  `✅ Delimitador TAB detectado: ${resultTab.meta.delimiter === "\t" ? "SIM" : "NÃO"}`,
);
...
console.log(
  `✅ Header detectado: ${resultSemHeader.meta.hasHeader ? "SIM" : "NÃO"}`,
);
console.log(
  `📊 Formato retorno: ${Array.isArray(resultSemHeader.data[0]) ? "Array de arrays" : "Objetos"}`,
);
...
console.log(
  `📭 CSV vazio - Linhas: ${resultVazio.meta.totalRows}, Erros: ${resultVazio.meta.errors.length}`,
);

const csvSoHeader = "Nome;Idade;Cidade";
const resultSoHeader = parseCSV(csvSoHeader);
console.log(
  `🏷️ Só header - Linhas: ${resultSoHeader.meta.totalRows}, Header: ${resultSoHeader.meta.hasHeader}`,
);
...
console.log(
  `🚀 Processadas ${resultPerformance.meta.totalRows} linhas em alta velocidade!`,
);
...
console.log(
  `🔧 Com pipe forçado: ${resultOpcoes1.meta.delimiter} | Headers: ${resultOpcoes1.meta.hasHeader}`,
);
...
console.log(
  `🔧 Header forçado: ${Object.keys(resultOpcoes2.data[0] || {}).join(", ")}`,
);

console.log("\n🎉 === DEMONSTRAÇÃO CONCLUÍDA ===");
console.log("✅ Função parseCSV testada em todos os cenários!");

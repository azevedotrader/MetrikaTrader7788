    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-black border border-zinc-700 rounded-lg p-3 text-white">
          ...
          <p className={`${data.value >= 0 ? 'text-green-600' : 'text-red-400'}`}>
            💰 Total: R$ {data.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
          </p>
          <p className={`${data.average >= 0 ? 'text-green-600' : 'text-red-400'}`}>
                    ...
                    <div key={market} className="flex justify-between items-center text-xs">
                      ...
                      <div className="text-right">
                        <div className={`font-bold ${
                          data.total >= 0 ? 'text-green-600' : 'text-red-400'
                        }`}>
    ...
    container.innerHTML = `
      <div class="flex gap-1">
        ...
        <div class="bg-zinc-800/90 rounded-lg border border-zinc-700 p-1.5 w-20 h-16 flex flex-col justify-center items-center text-center">
          ...
          <div class="text-xs font-bold truncate ${finalAccumulated >= 0 ? 'text-green-600' : 'text-red-400'}">
            ...
            content={({ active, payload, label }) => {
              ...
              return (
                <div className="bg-zinc-900/95 backdrop-blur-sm border border-zinc-700 rounded-lg p-3 shadow-xl">
                  ...
                  <div className={`text-sm font-semibold ${isPositive ? 'text-green-600' : 'text-red-400'}`}>
        ...
        {recentTrades.map((trade, index) => {
          ...
          return (
            ...
            >
              ...
              <div className={`font-medium ${result >= 0 ? 'text-green-600' : 'text-red-400'}`}>
            ...
            <Card className="bg-zinc-900/90 border-zinc-800 hover:bg-zinc-900/95 transition-colors" data-testid="card-net-pnl">
              <CardContent className="p-2 md:p-3 lg:p-4">
                ...
                <div className={`text-lg md:text-xl lg:text-2xl font-bold ${(() => {
                  ...
                  return totalResult >= 0 ? 'text-green-600' : 'text-red-400';
                })()} break-words`}>
                  R$ {(() => {
                    const totalResult = filteredTrades.reduce((sum, t) => sum + parseFloat(t.resultado || '0'), 0);
              ...
              <CardContent className="p-2 md:p-3 lg:p-4">
                ...
                <div className="flex items-center justify-between gap-2 sm:gap-3">
                  <div className={`text-2xl md:text-3xl lg:text-4xl font-bold ${(() => {
                    ...
                    return winRate >= 60 ? 'text-green-600' : winRate >= 40 ? 'text-yellow-400' : 'text-red-400';
                  })()}`}>
                    {(() => {
                      const winTrades = filteredTrades.filter(t => parseFloat(t.resultado || '0') > 0).length;
                      const totalTrades = filteredTrades.length;
                  ...
                  <div className="shrink-0 min-w-[35px]">
                    <CircularProgress 
                      ...
                      color={(() => {
                        ...
                        const totalTrades = filteredTrades.length;
                        const winRate = totalTrades > 0 ? (winTrades / totalTrades) * 100 : 0;
                        return winRate >= 60 ? "#16a34a" : winRate >= 40 ? "#f59e0b" : "#ef4444";
                      })()}
                    />
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-1 gap-3 h-full">
                ...
                <div className="bg-zinc-800/90 rounded-lg border border-zinc-700 p-3 flex flex-col items-center text-center">
                  ...
                  <div className={`text-lg md:text-xl font-bold mb-2 ${(() => {
                    ...
                    return avgResult >= 0 ? 'text-green-600' : 'text-red-400';
                  })()}`}>
                    R$ {(() => {
                      const totalResult = periodFilteredTrades.reduce((sum, t) => sum + parseFloat(t.resultado || '0'), 0);
                      const avgResult = periodFilteredTrades.length > 0 ? totalResult / periodFilteredTrades.length : 0;
